...
var connectionString = function connectionString() {
...
var getSeriesFromJSON = function getSeriesFromJSON(data) {
    var results = [];
...
var loadSettings = function loadSettings() {
    ...
    if (cs != null) {
        connectionSettings = JSON.parse(cs);
    }
...
var hideModalError = function hideModalError() {
    $("div#modal-error").empty().hide();
};

// show errors within the Write Data modal
var showModalError = function showModalError(message) {
    hideModalSuccess();
...
var showModalSuccess = function showModalSuccess(message) {
    hideModalError();
...
var hideQueryError = function hideQueryError() {
    $("div#query-error").empty().hide();
};

// show errors from queries
var showQueryError = function showQueryError(message) {
    hideQuerySuccess();
...
var hideQuerySuccess = function hideQuerySuccess() {
    $("div#query-success").empty().hide();
};

// show success messages from queries
var showQuerySuccess = function showQuerySuccess(message) {
    hideQueryError();
...
var hideDatabaseWarning = function hideDatabaseWarning() {
    $("div#database-warning").empty().hide();
};

// show warning from database lookup
var showDatabaseWarning = function showDatabaseWarning(message) {
...
var handleSubmit = function handleSubmit(e) {
    ...
    query.done(function (data) {
        ...
        if (values == null || values.length == 0) {
            ...
            hideDatabaseWarning();
            React.render(React.createElement(DataTable, { series: series }), document.getElementById('table'));
        }
    });
...
var handleRequestError = function handleRequestError(e) {
    var errorText = e.status + " " + e.statusText;
    showDatabaseWarning("Unable to fetch list of databases.");

    if ("responseText" in e) {
        try {
            errorText = "Server returned error: " + JSON.parse(e.responseText).error;
        } catch (e) {}
    }
...
var stringifyTags = function stringifyTags(tags) {
    var tagStrings = [];
...
var DataTable = React.createClass({
    ...
    render: function render() {
        var tables = this.props.series.map(function (series) {
            return React.createElement("div", null, React.createElement("h1", null, series.name), React.createElement("h2", null, stringifyTags(series.tags)), React.createElement("table", { className: "table" }, React.createElement(TableHeader, { data: series.columns }), React.createElement(TableBody, { data: series })));
        });
...
var pretty = function pretty(val) {
...
var getClientVersion = function getClientVersion() {
...
var chooseDatabase = function chooseDatabase(databaseName) {
...
var updateDatabaseList = function updateDatabaseList() {
