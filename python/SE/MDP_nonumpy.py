...
class MDP:
	...
	for i in range(len(self.statequeue)):
	    ...
	    fout.write(str(entropy(state.negprior))+' ')

	    # model entropy
	    fout.write(str(entropy(state.posmodel))+' ')
	    fout.write(str(entropy(state.negmodel))+' ')
	    ...
	    irrel = cross_entropy(state.posmodel,self.back) - \
		    0.1*cross_entropy(state.negmodel,self.back)
		...
		leng = doclengs[i]
		if i<10:
		    WIG10 += (state.ret[i]-irrel)/10.0
		    NQC10 += math.pow(state.ret[i]-irrel,2)/10.0
		    ...
		    WIG20 += (state.ret[i]-irrel)/10.0
		    NQC20 += math.pow(state.ret[i]-irrel,2)/10.0
		    for wordID,prob in doc.iteritems():
	    ...
	    # query feedback
	    oret = retrieveCombination(state.posprior,state.negprior,\
		    self.back,self.inv_index,self.doclengs,\
		    self.alpha_d,0.1)[:50]
	    qf10 = 0.0
	    qf20 = 0.0
	    qf50 = 0.0
	    for docID1,rel1 in oret[:10].iteritems():
		for docID2,rel2 in state.ret[:10]:
		    if docID1==docID2:
			qf10 += 1.0
			break
	    for docID1,rel1 in oret[:20].iteritems():
		for docID2,rel2 in state.ret[:20]:
		    if docID1==docID2:
			qf20 += 1.0
			break
	    for docID1,rel1 in oret[:50].iteritems():
		for docID2,rel2 in state.ret[:50]:
		    if docID1==docID2:
			qf50 += 1.0
			break
	    fout.write(str(qf10)+' ')
